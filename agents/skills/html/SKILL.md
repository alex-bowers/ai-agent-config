---
name: html
description: Writes and reviews semantic, accessible HTML and template markup that stays readable and low-noise. Use when creating or refactoring HTML or Vue templates, cleaning up div soup, choosing better elements, improving form markup, fixing heading or landmark structure, or replacing custom controls with native HTML.
metadata:
...
- cleaning up Vue component template markup without changing its behavior
...
When the markup lives inside Vue or another HTML-like template language:
- treat the file as template markup, not as a plain standalone HTML document
- preserve framework syntax such as `v-if`, `v-for`, `v-else`, `v-html`, `v-bind`, `:class`, `:style`, event attributes like `@click`, and Vue special elements like `<Teleport>`, `<Transition>`, and `<Suspense>`
...
- in Vue, treat template and lint accessibility warnings as useful guardrails and do not add ignore comments casually
...
- rewriting valid Vue directives or template blocks as if they were broken HTML
...
- would this cleanup break a Vue directive, binding, loop, key, or component boundary?
...
- in Vue or other template files, preserve directives, template syntax, bindings, and special elements
